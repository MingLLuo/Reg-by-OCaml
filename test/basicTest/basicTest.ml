  ...
  let m3 =
    {
      qs = [ 0; 1; 2; 3 ];
      sigma = [ 'a'; 'b' ];
      delta = [ (0, None, 1); (1, Some 'b', 2); (2, Some 'b', 3) ];
      q0 = 0;
      fs = [ 3 ];
    }
  in
  assert_nfa_deny m3 "";
  assert_nfa_accept m3 "bb";
...
let test_nfa_move _ =
  ...
  in
  assert_nfa_move m5 [ 0 ] None [ 1; 0 ];
  assert_nfa_move m5 [ 0; 1 ] None [ 2; 1; 0 ];
  assert_nfa_move m5 [ 3 ] None [];
  assert_nfa_move m5 [ 0 ] (Some 'q') []
...
let test_str_to_nfa_empty _ =
  ...
  let m2 = regexp_to_nfa @@ string_to_regexp "((E)|(E))*" in
